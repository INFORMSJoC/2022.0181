...
cal_loss_once <- function(ii) 
{
 ...
 p= settings[[idx]]
  ...
  file_name_BC <- paste(set,p,snr, "BC.txt", sep = "_")
  file_path_BC <- paste(path_result, file_name_BC, sep = "")
  
  file_name_rank <- paste(set,p,snr, "rank.txt", sep = "_")
  ...
  file_name_mask <- paste(set,p,snr, "mask.txt", sep = "_")
  file_path_mask <- paste(path_result, file_name_mask, sep = "")
  
  file_name_swamp <- paste(set,p,snr, "swamp.txt", sep = "_")
  file_path_swamp <- paste(path_result, file_name_swamp, sep = "")
  
  file_name_detection <- paste(set,p,snr, "detection.txt", sep = "_")
  file_path_detection <- paste(path_result, file_name_detection, sep = "")
  
  # debug
  file_name_debug <- paste(set,p,snr, "debug.txt", sep = "_")
...
#snr_list = c(0.25,0.5,0.75,1,1.25,1.5)
p_list = c(500,800,1000,1200,1500,1800,2000)
settings <- lapply(p_list, unlist, 1)
