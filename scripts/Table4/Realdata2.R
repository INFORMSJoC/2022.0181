# just return the results in Results-Sim22_12_15_rollingwindow
########### read table
rm(list = ls())
...
once_function = function(t)
{
  ...
  if(0<t & t<(rol+1))
  {
    ...
    set.seed(t)
    start4 = Sys.time()
    re4 = RS2(Y_train,X_train,rmax = NULL, r =8,outlier = TRUE,alpha = 0.6) 
    stop4 = Sys.time()
    ...
    Sres_rop =   (Y_true - Ytest4)^2  
    PE_rop = norm(Y_true - Ytest4,"2")/norm(Y_true,"2")
    Sys.sleep(runif(5, min=0, max=1))
    #cat(Sres_rop, "\n", file = file_path_Sres_rop, append = TRUE)
    data.table::fwrite(as.list(Sres_rop), file = file_path_Sres_rop, append=TRUE, row.names=FALSE, col.names=FALSE)
    ...
    set.seed(t)
    start5 = Sys.time()
    re4 = RS2(Y_train,X_train,rmax = NULL, r =9,outlier = TRUE,alpha = 0.6) 
    rank.est4 = re4 $rank
    B.est4 = re4$B
    C.est4 = re4$C  
    
    Y1 = Y_train - C.est4
    re5 = RS2(Y1,X_train,rmax = NULL, r =8,outlier = FALSE,alpha = 0.6) 
    B.est5 = re5$B
    stop5 = Sys.time()
    timecost5 = stop5 - start5 # time record
    
    Ytest5 = X_test %*% B.est5
    Sres_ropr =   (Y_true - Ytest5)^2  
    PE_ropr = norm(Y_true - Ytest5,"2")/norm(Y_true,"2")
    Sys.sleep(runif(5, min=0, max=1))
    #cat(Sres_ropr, "\n", file = file_path_Sres_ropr, append = TRUE)
    data.table::fwrite(as.list(Sres_ropr), file = file_path_Sres_ropr, append=TRUE, row.names=FALSE, col.names=FALSE)
...
sfLapply(seq_len(7*rol), once_function)
sfStop()