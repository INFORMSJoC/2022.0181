...
snr = 0.75
RE_B = array(dim = c(2,length(p_list),100)) 
RE_C  = array(dim = c(2,length(p_list),100)) 
RE_time  = array(dim = c(2,length(p_list),100)) 
i = 1
for (p in p_list) {
  print(p)
  p = p
  
  file_name_B <- paste(set,p,snr, "B.txt", sep = "_")
  ...
  file_name_C <- paste(set,p,snr, "C.txt", sep = "_")
  ...
  file_name_time <- paste(set,p,snr, "time.txt", sep = "_")
...
da1=c(RE_B[2,1,],RE_B[2,2,],RE_B[2,3,],RE_B[2,4,],RE_B[2,5,],RE_B[2,6,],RE_B[2,7,])
noutl=factor(rep(c(500,800,1000,1200,1500,1800,2000),each=100))
data1 = data.frame(da1, noutl )
P1 = ggplot( data1,aes(x = noutl, y = da1)) +
  geom_boxplot(alpha=0.7) +
  coord_cartesian(ylim = c(0,0.1))+
  scale_y_continuous(name=TeX("$Err(\\hat{B})$"))+
  scale_x_discrete(name = expression(p))

da2=c(RE_C[2,1,],RE_C[2,2,],RE_C[2,3,],RE_C[2,4,],RE_C[2,5,],RE_C[2,6,],RE_C[2,7,])
nout2=factor(rep(c(500,800,1000,1200,1500,1800,2000),each=100))
